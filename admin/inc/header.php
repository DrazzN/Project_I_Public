<?php
// require_once('sess_auth.php');

?>

<head>
    <meta charset="UTF-8" />
    <meta http-equiv="X-UA-Compatible" content="IE=edge" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Course · e-learnings</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-EVSTQN3/azprG1Anm3QDgpJLIm9Nao0Yz1ztcQTwFspd3yD65VohhpuuCOmLASjC" crossorigin="anonymous">
    <?php define('base_url', 'http://localhost/Project_IPublic/'); ?>
    <link href="http://localhost/Project_I_Public/bootstrap/css/bootstrap.css" rel="stylesheet" />
    <link href="http://localhost/Project_I_Public/bootstrap/css/carousel.css" rel="stylesheet">
    <style>
        .bd-placeholder-img {
            font-size: 1.125rem;
            text-anchor: middle;
            -webkit-user-select: none;
            -moz-user-select: none;
            user-select: none;
        }

        @media(min-width: 780px) {
            .form-input {
                width: 45%;
            }

            .bd-placeholder-img-lg {
                font-size: 3.5rem;
            }
        }

        .img row {
            background: white;
            border-radius: 30px;
            box-shadow: 12px 12px 22px;
        }

        body, html{
            height: 100%;
        }

        .bg {
            background-image: url('http://localhost/Project_I_Public/img/lg-back.webp');
            height: 100%;
            background-position: center;
            background-repeat: no-repeat;
            background-size: cover;
        }

        .text-box {
            font-size: 20px;
            border-bottom: 1px solid #4caf50;
        }

        .far {
            font-size: 8.5rem;
            color: #00d9ff;
            animation: rotation 2s infinite linear;
        }

        .nav li img,
        .logo, .quote {
            width: 30px;
            height: 30px;
        }

        @keyframes rotation {
            from {
                transform: rotate(0deg);
            }

            to {
                transform: rotate(359deg);
            }
        }

        a {
            text-decoration: none;
            color: #000000;
        }

        .btn-mycolorpic {
            background: #6c5ce7;
        }

        .text-mycolorpic {
            color: #6c5ce7;
        }
        .quote {
            width: 20px;
        }
    </style>
    <!--<link href="http://localhost/Project_I_Public/bootstrap/css/sidebars.css" rel="stylesheet">
    -->


</head>